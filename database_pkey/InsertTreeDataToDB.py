...
This code matches each tuple in Trees.shp to the pkey and inserts the data in sidewalks_incline.geojson
to SQL database.

File(s) required:
    1. sidewalks_incline.geojson (contains the pkey and incline)
    2. Trees.shp (tree data)
'''
...
try:
    ...
    sys.exit('Error while loading Trees.shp')
...
for i in range(0 , len(imported_data)):
    print('tree processing', i, 'our of', len(imported_data))
    ...
    if result:
        sql_command = (
                      ...
                      str(i + 1) + ', \'' + imported_data['UNITID'][i] + '\', \'' + imported_data['GENUS'][i] + '\', '+
                      str(result[0]) +
                      ');'
                      )
        cursor.execute(sql_command)

cursor.execute('CREATE INDEX p_index ON trees(pkey) USING HASH')
cursor.close()
db.close()
